...
SYS_CHOWN					equ 92	;	const char *filename	uid_t user	gid_t group			
SYS_FCHOWN					equ 93	;	unsigned int fd	uid_t user	gid_t group			
SYS_LCHOWN					equ 94	;	const char *filename	uid_t user	gid_t group			
...
SYS_SETREGID				equ 114	;	gid_t rgid	gid_t egid				
...
SYS_GETRESGID				equ 120	;	gid_t *rgid	gid_t *egid	gid_t *sgid			
...
SYS_MKNOD					equ 133	;	const char *filename	umode_t mode	unsigned dev			
...
SYS_KCMP					equ 312	;	pid_t pid1	pid_t pid2	int type	unsigned long idx1	unsigned long idx2	
SYS_FINIT_MODULE			equ 313	;	int fd	const char __user *uargs	int flags			
SYS_SCHED_SETATTR			equ 314	;	pid_t pid	struct sched_attr __user *attr	unsigned int flags			
SYS_SCHED_GETATTR			equ 315	;	pid_t pid	struct sched_attr __user *attr	unsigned int size	unsigned int flags		
SYS_RENAMEAT2				equ 316	;	int olddfd	const char __user *oldname	int newdfd	const char __user *newname	unsigned int flags	
SYS_SECCOMP					equ 317	;	unsigned int op	unsigned int flags	const char __user *uargs			
SYS_GETRANDOM				equ 318	;	char __user *buf	size_t count	unsigned int flags			
SYS_MEMFD_CREATE			equ 319	;	const char __user *uname_ptr	unsigned int flags				
SYS_KEXEC_FILE_LOAD			equ 320	;	int kernel_fd	int initrd_fd	unsigned long cmdline_len	const char __user *cmdline_ptr	unsigned long flags	
SYS_BPF						equ 321	;	int cmd	union bpf_attr *attr	unsigned int size			
STUB_EXECVEAT				equ 322	;	int dfd	const char __user *filename	const char __user *const __user *argv	const char __user *const __user *envp	int flags	
USERFAULTFD					equ 323	;	int flags					
MEMBARRIER					equ 324	;	int cmd	int flags				
MLOCK2						equ 325	;	unsigned long start	size_t len	int flags			
COPY_FILE_RANGE				equ 326	;	int fd_in	loff_t __user *off_in	int fd_out	loff_t __user * off_out	size_t len	unsigned int flags
PREADV2						equ 327	;	unsigned long fd	const struct iovec __user *vec	unsigned long vlen	unsigned long pos_l	unsigned long pos_h	int flags
PWRITEV2					equ 328	;unsigned long fd	const struct iovec __user *vec	unsigned long vlen	unsigned long pos_l	unsigned long pos_h	int flags
...
; standart argument passing: rax = (%rax, %rbx, %rcx, %rdx)
section .text

; int fopen(str file, int flags, int mode)
fopen:
	push rdx
	push rcx
...
; int fprints(int fd, str msg)
; prints new line additionally
fprints_lf:
	push rax ; fd
	call fprints

	mov rbx, rax ; b = returnd
	pop rax	; fd
	push rbx ; returned
...
; parse int into string, in the stack
; print_uint64(int fd, int num)
print_uint64:
	push rax
	push rbx
	mov rax, rbx

	mov rbx, _print_uint64_buf+9
	mov rcx, 10
	mov rdi, 10
.loop:
	mov rdx,0
...
; int fscans(int fd, str buffer, int len)
fscans:
	push rbx
	push rcx
	push rdx
	...
	ret
section .data
	_print_uint64_buf db '0000000000',10,0

section .rodata
	err_read: db "Read error!", 0Ah, 0h
	err_read_s: equ $-err_read
	err_write: db "Write error!", 0Ah, 0h
	err_write_s: equ $-err_write
	err_open: db "open error!", 0Ah, 0h
	err_open_s: equ $-err_open
	err_close: db "close error!", 0Ah, 0h
	err_close_s: equ $-err_close
...
.nextArg:
	...
	pop rax ; the string argument --> file name
	push rcx
	...
	xor rcx, rcx ; rcx - word count
	mov rdx, 1 ; rdx - if previous char is a whitespace
.scanLoop:
		push rax		
	...
	.nextChar:
		...
		; if(bl == 32 || (bl >= 9 && bl <= 13)) {.whitespace} else {.not_whitespace}
			cmp bl, 32
			je .whitespace
			cmp bl, 9
			jb .not_whitespace
